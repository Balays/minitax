...
has_surrounding_quotes <- function(value) grepl("^(['\"]).*\\1$", trimws(value))
...
if (length(attr(cfg, "duplicates")) > 0) warn("Duplicate config key(s); last value wins: " %+% paste(attr(cfg, "duplicates"), collapse = ", "))
...
require_keys <- function(keys) {
  ...
  if (length(missing) > 0) err("Missing required config argument(s): " %+% paste(missing, collapse = ", ")) else ok("Required config arguments are present: " %+% paste(keys, collapse = ", "))
}
check_no_quotes <- function(name) {
  value <- get_cfg(name)
  if (!is_config_na(value) && has_surrounding_quotes(value)) err("Config value '" %+% name %+% "' has surrounding quotes. Use " %+% trim_quotes(value) %+% " instead.")
}
...
check_integer <- function(name, min_value = NULL, max_value = NULL, required = TRUE) {
  ...
  if ((is.null(min_value) || number >= min_value) && (is.null(max_value) || number <= max_value)) ok(name %+% " = " %+% number)
  invisible(number)
}
check_index_batch_size <- function(name, default = "128G") {
  value <- get_cfg(name, default)
  if (is_config_na(value)) value <- default
  if (!grepl("^[0-9]+[KkMmGgTt]?$", value)) err(name %+% " must look like 128G, 64000M, or 500000000.") else ok(name %+% " = " %+% value)
}
...
file_readable <- function(path, label, required = TRUE) {
  ...
  if (file.access(path, 4) == 0) ok(label %+% " is readable.") else if (required) err(label %+% " is not readable: " %+% path) else warn(label %+% " is not readable: " %+% path)
  invisible(TRUE)
}
check_command <- function(command, label, required = TRUE) {
  found <- Sys.which(command)
  if (nzchar(found)) ok(label %+% " found: " %+% found) else if (required) err(label %+% " not found in PATH.") else warn(label %+% " not found in PATH.")
  invisible(nzchar(found))
}
check_executable <- function(path, label, required = TRUE) {
  path <- normalize_config_path(path)
  if (!is_config_na(path) && !grepl("[/\\\\]", path)) return(check_command(path, label, required = required))
  if (!path_exists(path, label, required = required)) return(invisible(FALSE))
  if (file.access(path, 1) == 0) ok(label %+% " is executable.") else warn(label %+% " exists but is not marked executable: " %+% path)
  invisible(TRUE)
}
...
check_fastq_inputs <- function() {
  ...
  if (platform == "Illumina" && reads == "paired") {
    ...
    if (length(r1) > 0 && length(r2) > 0 && length(missing_r1) == 0 && length(missing_r2) == 0) ok("Illumina paired FASTQ files look matched for " %+% length(r1) %+% " sample(s).")
  } else {
...
check_mapq <- function() {
  ...
  if (length(parts) == 2 && parts[[1]] > parts[[2]]) err("mapq.filt range start must be <= range end.") else ok("mapq.filt is valid: " %+% value)
}
...
check_methods <- function() {
  ...
  if (length(bad) > 0) err("Unknown method(s): " %+% paste(bad, collapse = ", ")) else ok("Method list is valid: " %+% paste(methods, collapse = ", "))
  thresholds <- split_values(get_cfg("BestAln_thresholds"), default = "0.6")
  nums <- suppressWarnings(as.numeric(thresholds))
  if (any(is.na(nums))) err("BestAln_thresholds must be numeric.") else if (!length(nums) %in% c(1, 7)) err("BestAln_thresholds must contain either one value or seven rank-level values.") else if (any(nums < 0 | nums > 1)) warn("BestAln_thresholds are usually probabilities between 0 and 1.") else ok("BestAln_thresholds is valid.")
}
check_output_dir <- function() {
  ...
  if (is_config_na(outdir)) outdir <- paste("minitax", project, platform, vregion, db, sep = "_")
  outdir <- normalize_config_path(outdir)
  parent <- dirname(outdir)
  if (!dir.exists(parent)) warn("Output parent directory does not exist yet: " %+% parent) else if (file.access(parent, 2) != 0) err("Output parent directory is not writable: " %+% parent) else ok("Output parent directory is writable: " %+% parent)
  if (dir.exists(outdir) && length(list.files(outdir, all.files = TRUE, no.. = TRUE)) > 0) warn("Output directory already exists and is not empty: " %+% outdir) else ok("Output directory is available: " %+% outdir)
}
...
check_database <- function() {
  ...
  if (db == "all_NCBI_genomes") {
    ...
    required_files <- c("gtdb_ssu_taxonomy.tsv")
    optional_files <- character()
  } else {
  ...
  if (!is_config_na(index)) {
    ...
    if (file.exists(index_path)) {
      ...
      if (!is_config_na(mm2_ref)) ref_candidates <- unique(c(mm2_ref, file.path(db_dir, mm2_ref)))
      ref_found <- ref_candidates[file.exists(ref_candidates)]
      if (index_required && length(ref_found) > 0) warn("minimap2 index not found, but mm2_ref exists; minitax.sh can build the index: " %+% ref_found[[1]]) else file_readable(index_path, "minimap2 index", required = index_required)
    }
...
check_r_sources <- function() {
  ...
  required_sources <- file.path(minitax_dir, c("R/minitax.v2.R", "R/cigar_score.R", "R/cigar_to_length.R", "R/cigar.sum.R", "R/rename_duptaxa.R", "R/minitax.wrapfun.complete.R", "R/BestAln.R", "R/add_lineage.R"))
  misc_sources <- file.path(misc_dir, c("R/ov.from.bam2.R", "R/dt.from.bam.R", "R/get.best.aln.R", "R/misc.functions.R", "R/add_rightmost_non.NA_col.R", "R/paste_columns_dt.R", "R/bam.flags.tsv"))
  missing <- c(required_sources, misc_sources)[!file.exists(c(required_sources, misc_sources))]
  if (length(missing) == 0) ok("All required R source/helper files are present.") else err("Missing R source/helper file(s): " %+% paste(missing, collapse = ", "))
}
check_classify_inputs <- function() {
  ...
  if (step == "classify") {
    outdir <- get_cfg("outdir")
    if (is_config_na(outdir)) outdir <- paste("minitax", get_cfg("project", "project"), get_cfg("platform", "platform"), get_cfg("Vregion", "WGS"), get_cfg("db", "db"), sep = "_")
    outdir <- normalize_config_path(outdir)
    ...
    if (length(bam_files) == 0 && length(taxa_files) == 0) err("No BAM files or cached best_alignments_w_taxa files found for classification under: " %+% outdir) else ok("Classification inputs found: " %+% length(bam_files) %+% " BAM file(s), " %+% length(taxa_files) %+% " cached taxa file(s).")
  }
}

require_keys(c("platform", "db", "db.dir", "nproc"))
for (name in c("db", "db.dir", "indir", "outdir", "mm2_path", "mm2_index", "mm2_ref", "mapper_backend", "parabricks_image", "mm2_index_batch", "minitax.dir", "misc.dir")) check_no_quotes(name)
check_choice("platform", c("Illumina", "ONT", "PacBio"))
mapper_backend <- get_cfg("mapper_backend", "auto")
if (!mapper_backend %in% c("auto", "mm2-fast", "minimap2", "cpu", "parabricks")) err("mapper_backend must be one of: auto, mm2-fast, minimap2, cpu, parabricks.") else ok("mapper_backend = " %+% mapper_backend)
check_index_batch_size("mm2_index_batch")
parabricks_num_gpus <- get_cfg("parabricks_num_gpus", "1")
if (!grepl("^[0-9]+$", parabricks_num_gpus) || as.integer(parabricks_num_gpus) < 1) err("parabricks_num_gpus must be a positive integer.") else ok("parabricks_num_gpus = " %+% parabricks_num_gpus)
...
for (name in c("debug", "keep.highest.mapq.aln.only", "crop.na.tax", "multicore", "saveRAM", "reuse.taxa.cache", "keep.max.cigar", "best.mapq")) check_logical(name, required = FALSE)
check_mapq()
check_cigar_points()
check_methods()
check_output_dir()
check_disk_space()
if (step %in% c("all", "map", "database", "classify")) check_database()
if (step %in% c("all", "map")) {
  mm2_path <- get_cfg("mm2_path")
  mapper_backend <- get_cfg("mapper_backend", "auto")
  if (is_config_na(mm2_path) && mapper_backend != "parabricks") err("mm2_path is required by minitax.sh unless mapper_backend=parabricks.") else if (is_config_na(mm2_path)) warn("mm2_path is missing; mapper_backend=parabricks will not be able to build a missing minimap2 index.") else check_executable(mm2_path, "minimap2 executable", required = TRUE)
  if (mapper_backend == "parabricks") {
    mm2_ref <- get_cfg("mm2_ref")
    if (is_config_na(mm2_ref)) err("mapper_backend=parabricks requires mm2_ref to point to the reference FASTA.")
  ...
  platform <- get_cfg("platform")
  if (platform == "Illumina") check_choice("reads", c("paired", "merged"))
  check_fastq_inputs()
}
if (step %in% c("all", "classify")) check_classify_inputs()
